document.addEventListener("DOMContentLoaded", function () {
  ...
  function showEmptyState() {
    mainContent.innerHTML = `
            <div class="text-center" style="padding: 40px;">
                ...
                }" alt="Empty State" style="width: 150px; height: 150px; opacity: 0.5; margin-bottom: 20px;">
  ...
  function loadCounselors() {
    ...
    fetch((window.BASE_URL || "/") + `admin/counselors/api?t=${timestamp}`)
      ...
      .then((data) => {
        ...
        if (addCounselorBtn) {
          addCounselorBtn.style.display =
            counselors.length > 0 ? "flex" : "none";
        }
        ...
        const pending = counselors.filter(
          (c) => String(c.is_verified || 0) === "0"
        );
        const verified = counselors.filter(
          (c) => String(c.is_verified || 0) === "1"
        );

        const renderItem = (counselor, index, isPending) => `
                    <div class="counselor-item ${
                      index === 0 && pending.length > 0 ? "active" : ""
                    } ${isPending ? "pending" : ""}" data-id="${
          counselor.counselor_id
        }" data-index="${counselors.indexOf(counselor)}" style="${
          isPending ? "border-left:4px solid #f1c40f;" : ""
        }">
                        <img src="${
                          counselor.profile_picture
                            ? (window.BASE_URL || "/") +
                              "Photos/profile_pictures/" +
                              counselor.profile_picture.split("/").pop()
                            : (window.BASE_URL || "/") + "Photos/profile.png"
                        }" alt="${
          counselor.name
        }" class="counselor-avatar" onerror="this.onerror=null;this.src='${
          (window.BASE_URL || "/") + "Photos/profile.png"
        }'">
          ...
          isPending
            ? '<span title="Pending verification" style="display:inline-block;width:8px;height:8px;background:#f1c40f;border-radius:50%;"></span>'
            : ""
        }</h3>
        ...
        if (pending.length) {
          html +=
            '<div class="counselor-section-title" style="padding:6px 10px;color:#aa8400;font-weight:600;">Pending Verification</div>';
          html += pending.map((c, i) => renderItem(c, i, true)).join("");
        }
        if (verified.length) {
          html +=
            '<div class="counselor-section-title" style="padding:6px 10px;color:#555;font-weight:600;margin-top:6px;">Verified</div>';
        ...
        document.querySelectorAll(".counselor-item").forEach((button) => {
          button.addEventListener("click", function () {
            ...
            document
              .querySelectorAll(".counselor-item")
      ...
      .catch((error) => {
        ...
        showNotification(
          "Failed to load counselors: " + error.message,
          "error"
        );
        showEmptyState();
      });
  ...
  function handleSubmit(e) {
    ...
    formData.append(
      "specialization",
      document.getElementById("modal-specialization").value || ""
    );
    formData.append(
      "degree",
      document.getElementById("modal-degree").value || ""
    );
    formData.append(
      "email",
    ...
    formData.append(
      "contactNumber",
      document.getElementById("modal-contactNumber").value || ""
    );
    formData.append(
      "licenseNumber",
      document.getElementById("modal-licenseNumber").value || ""
    );
    formData.append(
      "address",
    ...
    fetch((window.BASE_URL || "/") + "admin/counselors/api", {
      ...
      .then((data) => {
        ...
        if (data.success) {
          showNotification(
            data.message || "Counselor updated successfully",
            "success"
          );
          closeModal();
                ...
                <img src="${
                  counselor.profile_picture
                    ? (window.BASE_URL || "/") +
                      ...
                      counselor.profile_picture.split("/").pop()
                    : (window.BASE_URL || "/") + "Photos/profile.png"
                }" alt="Counselor Profile" class="profile-image" id="main-profile-image" onerror="this.onerror=null;this.src='${
      (window.BASE_URL || "/") + "Photos/profile.png"
    }'">
            </div>
            ...
            <div class="details-right">
                <div class="form-grid">
                    <div class="form-group">
                        ...
                        <div class="info-display" id="counselorId">${
                          counselor.counselor_id || ""
                        }</div>
                    </div>
                    ...
                    <div class="form-group">
                        <label>Name:</label>
                        <div class="info-display" id="name">${
                          counselor.name || ""
                        }</div>
                    </div>
                    ...
                    <div class="form-group">
                        <label>Degree:</label>
                        <div class="info-display" id="degree">${
                          counselor.degree || ""
                        }</div>
                    </div>
                    ...
                    <div class="form-group">
                        <label>Email:</label>
                        <div class="info-display" id="email">${
                          counselor.email || ""
                        }</div>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="info-display" id="contactNumber">${
                    ...
                    <div class="form-group">
                        <label>Address:</label>
                        <div class="info-display" id="address">${
                          counselor.address || ""
                        }</div>
                    </div>
                </div>
    ...
    if (String(counselor.is_verified || 0) === "0") {
      ...
      if (approveBtn) {
        approveBtn.addEventListener("click", () =>
          approveCounselor(counselor.counselor_id, approveBtn)
        );
      }
      if (rejectBtn) {
  ...
  function openModal(counselor) {
    ...
    let endTime = "";
    if (
      counselorData.time_scheduled &&
      counselorData.time_scheduled.includes("-")
    ) {
      const parts = counselorData.time_scheduled
        .split("-")
        .map((s) => s.trim());
      startTime = parts[0] || "";
      endTime = parts[1] || "";
    ...
    modalContent.innerHTML = `
            <form id="updateCounselorForm">
                ...
                <div class="form-group">
                    <label>Counselor's ID:</label>
                    <input type="text" id="modal-counselorId" class="form-control" value="${
                      counselorData.counselor_id
                    }" ${isEditMode ? "readonly" : ""}>
                </div>
                
                <div class="form-group">
                    <label>Specialization:</label>
                    <input type="text" id="modal-specialization" class="form-control" value="${
                ...
                <div class="form-group">
                    <label>Name:</label>
                    <input type="text" id="modal-name" class="form-control" value="${
                      counselorData.name
                    }">
                </div>
                ...
                <div class="form-group">
                    <label>Degree:</label>
                    <input type="text" id="modal-degree" class="form-control" value="${
                ...
                <div class="form-group">
                    <label>Email:</label>
                    <input type="email" id="modal-email" class="form-control" value="${
                      counselorData.email
                    }">
                </div>
                ...
                <div class="form-group">
                    <label>Contact Number:</label>
                    <input type="text" id="modal-contactNumber" class="form-control" value="${
                      counselorData.contact_number
                    }">
                </div>
                
                <div class="form-group">
                    <label>License Number:</label>
                    <input type="text" id="modal-licenseNumber" class="form-control" value="${
                      counselorData.license_number
                    }">
                </div>
                    ...
                    <div class="days-selection" style="display: flex; gap: 10px; flex-wrap: wrap;">
                        <label style="display: flex; align-items: center; gap: 5px;">
                            <input type="checkbox" name="days" value="Monday" ${
                              counselorData.available_days?.includes("Monday")
                                ? "checked"
                                : ""
                            }>
                            Monday
                        ...
                        <label style="display: flex; align-items: center; gap: 5px;">
                            <input type="checkbox" name="days" value="Tuesday" ${
                              counselorData.available_days?.includes("Tuesday")
                                ? "checked"
                                : ""
                            }>
                            Tuesday
                        ...
                        <label style="display: flex; align-items: center; gap: 5px;">
                            <input type="checkbox" name="days" value="Wednesday" ${
                              counselorData.available_days?.includes(
                                "Wednesday"
                              )
                                ? "checked"
                                : ""
                        ...
                        <label style="display: flex; align-items: center; gap: 5px;">
                            <input type="checkbox" name="days" value="Thursday" ${
                              counselorData.available_days?.includes("Thursday")
                                ? "checked"
                                : ""
                            }>
                            Thursday
                        ...
                        <label style="display: flex; align-items: center; gap: 5px;">
                            <input type="checkbox" name="days" value="Friday" ${
                              counselorData.available_days?.includes("Friday")
                                ? "checked"
                                : ""
                            }>
                            Friday
    ...
    document
      ...
      .addEventListener("change", handleProfilePictureChange);
    ...
    fetch((window.BASE_URL || "/") + "admin/counselors/api", {
      ...
      .then((data) => {
        if (data.success) {
          ...
          const deletedCounselorElement = document.querySelector(
            `[data-id="${counselorId}"]`
          );
          ...
          const index = counselors.findIndex(
            (c) => c.counselor_id === counselorId
          );
          if (index !== -1) {
          ...
          const currentDisplayedId =
            document.getElementById("counselorId").textContent;
          ...
          showNotification(
            "Error deleting counselor: " + data.message,
            "error"
          );
        }
      })
  ...
  function loadCounselorSchedule(counselorId) {
    ...
    const url =
      (window.BASE_URL || "/") +
      `admin/counselor-info/schedule?counselor_id=${counselorId}&_=${Date.now()}`;
  ...
  function displayCounselorSchedule(schedule) {
    ...
    sortedDays.forEach((day) => {
      ...
      const formattedTimes = times.length
        ? formatTimeSlotsForBadges(times).join(", ")
        : "All day";
      const card = document.createElement("div");
      card.className = "availability-card";
      card.style.cssText =
        "border:1px solid #e0e0e0;border-radius:6px;padding:6px 10px;background:#fff;";
  ...
  function initializeCollapsed() {
    if (window.matchMedia("(max-width: 991.98px)").matches) {
      ...
      const topOffset = (headerRect ? headerRect.height : 50) + 10;
      if (toggler) {
  ...
  const searchInput = document.getElementById("counselorSearchInput");
  ...
  function filterCounselors(searchTerm) {
    ...
    // Get all counselor items and section titles
    const items = counselorList.querySelectorAll(".counselor-item");
    const sections = counselorList.querySelectorAll(".counselor-section-title");

    let visibleCount = 0;
    let pendingVisible = 0;
    let verifiedVisible = 0;

    // Filter counselor items
    items.forEach((item) => {
      const name =
        item.querySelector(".counselor-info h3")?.textContent.toLowerCase() ||
        "";
      const id =
        item.querySelector(".counselor-info p")?.textContent.toLowerCase() ||
        "";
    ...
    sections.forEach((section) => {
      const isPendingSection = section.textContent.includes("Pending");
      if (isPendingSection) {
        section.style.display = pendingVisible > 0 ? "block" : "none";
      } else {
        section.style.display = verifiedVisible > 0 ? "block" : "none";
      }
    });

    // Show "no results" message if needed
    const existingNoResults = counselorList.querySelector(
      ".no-results-message"
    );
    if (visibleCount === 0 && searchTerm) {
      if (!existingNoResults) {
        ...
        noResultsMsg.innerHTML = `
                  ...
                  <p>No counselors found matching "<strong>${escapeHtml(
                    searchTerm
                  )}</strong>"</p>
              `;
        counselorList.appendChild(noResultsMsg);
      }
