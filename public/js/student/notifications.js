...
(function() {
    ...
    function loadNotifications() {
        fetch(window.BASE_URL + 'student/notifications/all', {
            method: 'GET',
            credentials: 'include',
            headers: {
        ...
        .then(data => {
            if (data.status === 'success') {
                ...
                notifications = notifications.filter(n => !(n && n.type === 'message'));
    ...
    function setupNotificationItemListeners(item, notification, notificationId, notificationType, relatedId) {
        ...
        if (notification.is_read && notification.is_read !== 0 && notification.is_read !== false) {
            item.addEventListener('click', function(e) {
                // Don't trigger if clicking on buttons or checkbox
                if (e.target.closest('.notification-actions') || e.target.closest('.notification-checkbox')) {
                    return;
    ...
    function performDeleteSingle(notificationId, itemElement) {
        fetch(window.BASE_URL + 'student/notifications/delete', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-Requested-With': 'XMLHttpRequest'
    ...
    function redirectToNotificationPage(notificationType) {
        ...
        switch (notificationType) {
            case 'appointment':
                redirectUrl = baseUrl + 'student/my-appointments';
                break;
            case 'announcement':
            case 'event':
                redirectUrl = baseUrl + 'student/announcements';
                break;
            case 'follow-up':
            case 'follow_up_session':
                redirectUrl = baseUrl + 'student/follow-up-sessions';
                break;
            default:
