    ...
    .map(
      (appointment) => `
        <div class="appointment-card">
            ...
            <button class="follow-up-btn" onclick="openFollowUpSessionsModal(${
              appointment.id
            }, '${appointment.student_id}')">
...
async function openFollowUpSessionsModal(parentAppointmentId, studentId) {
  ...
  try {
    ...
    const response = await fetch(
      (window.BASE_URL || "/") +
        `counselor/follow-up/sessions?parent_appointment_id=${parentAppointmentId}`,
      {
        method: "GET",
        credentials: "include",
        headers: {
    ...
    if (data.status === "success") {
      ...
      try {
        ...
        if (label) {
          ...
          const card = document
            .querySelector(
              `.appointment-card button.follow-up-btn[onclick*="(${parentAppointmentId},"]`
            )
      ...
      (session) => `
        <div class="follow-up-session-card">
            <div class="session-header">
                <div class="session-sequence">Follow-up #${
                  session.follow_up_sequence
                }</div>
        ...
        session.status
            ...
            <div class="session-details">
                ...
                ${
                  session.reason
                    ? `<div class="session-reason"><strong>${
                        session.status === "cancelled"
                          ? "Reason For Cancellation:"
                          : "Reason For Follow-up:"
                      }</strong> ${session.reason}</div>`
                    : ""
                }
            </div>
            ${
...
async function markFollowUpCompleted(id) {
  ...
  const button = document.querySelector(
    `button[onclick*="markFollowUpCompleted(${id})"]`
  );
  ...
  try {
    ...
    const response = await fetch(
      (window.BASE_URL || "/") + "counselor/follow-up/complete",
      {
        method: "POST",
        credentials: "include",
        headers: {
    ...
    if (data.status === "success") {
      ...
      if (currentParentAppointmentId) {
        await openFollowUpSessionsModal(
          currentParentAppointmentId,
          currentStudentId
        );
      }
...
function openCancelFollowUpModal(id) {
  ...
  const modal = new bootstrap.Modal(
    document.getElementById("cancelFollowUpModal")
  );
  modal.show();
}
...
async function confirmCancelFollowUp() {
  ...
  try {
    ...
    const response = await fetch(
      (window.BASE_URL || "/") + "counselor/follow-up/cancel",
      {
        method: "POST",
        credentials: "include",
        headers: {
...
async function fetchAvailabilityByWeekday() {
  ...
  const now = Date.now();
  if (
    availabilityCache.weekdaysWithSlots &&
    availabilityCache.lastFetch &&
  ...
  try {
    const response = await fetch(
      (window.BASE_URL || "/") + "counselor/follow-up/availability-by-weekday",
      {
        method: "GET",
        credentials: "include",
        headers: {
...
async function isDateFullyBooked(dateStr, timeSlots) {
  ...
  try {
    ...
    const response = await fetch(
      (window.BASE_URL || "/") +
        `counselor/follow-up/booked-times?date=${dateStr}`,
      {
        method: "GET",
        credentials: "include",
        headers: {
    ...
    return (
      allRanges.length > 0 && allRanges.every((range) => bookedSet.has(range))
    );
  } catch (error) {
  ...
  try {
      ...
      const batchPromises = batch.map(async (dateStr) => {
        try {
          ...
          const availResponse = await fetch(
            (window.BASE_URL || "/") +
              `counselor/follow-up/availability?date=${dateStr}`,
            {
              method: "GET",
              credentials: "include",
              headers: {
          ...
          if (availResponse.ok) {
            ...
            if (
              availData.status === "success" &&
              Array.isArray(availData.time_slots)
            ) {
              const fullyBooked = await isDateFullyBooked(
                dateStr,
                availData.time_slots
              );
              if (fullyBooked) {
                return dateStr;
              }
    ...
    dateInput.addEventListener("change", function () {
      ...
      const disabledDates = JSON.parse(
        this.getAttribute("data-disabled-dates") || "[]"
      );
...
function openCreateFollowUpModal() {
  ...
  document.getElementById("parentAppointmentId").value =
    currentParentAppointmentId;
  ...
  // Show the modal
  const modal = new bootstrap.Modal(
    document.getElementById("createFollowUpModal")
  );
  modal.show();
}
...
async function saveFollowUp() {
  ...
  try {
    const response = await fetch(
      (window.BASE_URL || "/") + "counselor/follow-up/create",
      {
        method: "POST",
        credentials: "include",
        headers: {
...
function populateTimeOptionsForEdit(timeSlots, bookedTimes = []) {
  ...
  const bookedSet = new Set(bookedTimes);
  const filtered = incrementTimes.filter(
    (t) => !bookedSet.has(t) || t === currentValue
  );
...
async function updateFollowUp() {
  ...
  try {
    const response = await fetch(
      (window.BASE_URL || "/") + "counselor/follow-up/edit",
      {
        method: "POST",
        credentials: "include",
        headers: {
