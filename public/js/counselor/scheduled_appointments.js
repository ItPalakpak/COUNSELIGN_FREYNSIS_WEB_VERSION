document.addEventListener("DOMContentLoaded", function () {
  ...
  function loadAppointments() {
    ...
    const url =
      (window.BASE_URL || "/") +
      `counselor/appointments/scheduled/get?_=${Date.now()}`;
    fetch(url, {
      method: "GET",
      credentials: "include",
      headers: {
  ...
  function displayAppointments(appointments) {
    ...
    appointments.forEach((appointment) => {
      ...
      let scheduleType =
        ...
        (appointment.parent_appointment_id ? "Follow-up session" : "New");
      ...
      row.innerHTML = `
                ...
                <td>${
                  appointment.consultation_type || "Individual Consultation"
                }</td>
                <td>${scheduleType}</td>
    ...
    document.querySelectorAll(".mark-complete-btn").forEach((btn) =>
      btn.addEventListener("click", function () {
        updateAppointmentStatus(this.getAttribute("data-id"), "COMPLETED");
      })
    );
    ...
    document
      ...
      .addEventListener("click", function () {
        ...
        fetch(
          (window.BASE_URL || "/") +
            "counselor/appointments/updateAppointmentStatus",
          {
            method: "POST",
            body: formData,
            credentials: "include",
  ...
  function formatTime(time) {
    ...
    if (time.includes("-")) {
      ...
      return `${formatSingleTime(s)} - ${formatSingleTime(e)}`;
    }
  ...
  function showToast(title, message) {
    ...
    if (toast) toast.show();
    else new bootstrap.Toast(document.getElementById("statusToast")).show();
  }
  ...
  class AppointmentCalendar {
    ...
    renderCalendar() {
      ...
      SecureLogger.info("Calendar data:", {
        year,
        month,
        firstDay,
        daysInMonth,
      ...
      for (let day = 1; day <= daysInMonth; day++) {
        ...
        const appointmentCount =
          this.getAppointmentCountForDate(currentLoopDate);
        ...
        if (appointmentCount > 0) {
          ...
          dayElement.innerHTML = `
                    ...
                    `;
          dayElement.title = `${appointmentCount} appointment${
      ...
      SecureLogger.info(
        "Calendar rendering complete. Total days added:",
        daysInMonth
      );
    }
  }
  ...
  function loadCounselorSchedule() {
    ...
    const url =
      (window.BASE_URL || "/") +
      `counselor/appointments/schedule?_=${Date.now()}`;
  ...
  function displayCounselorSchedule(schedule) {
    ...
    if (!schedule || schedule.length === 0) {
      scheduleList.innerHTML =
        '<div class="schedule-row"><span class="text-muted">No schedule set</span></div>';
      return;
    }
    ...
    schedule.forEach((item) => {
      ...
      if (
        time &&
        time.trim() !== "" &&
        !groupedSchedule[day].includes(time.trim())
      ) {
