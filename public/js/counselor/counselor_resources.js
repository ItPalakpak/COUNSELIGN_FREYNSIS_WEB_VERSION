    ...
    fetch(url)
        ...
        .then(data => {
            if (data.success && data.resources && data.resources.length > 0) {
                counselorResourcesData = data.resources; // Store for preview module
...
document.addEventListener('click', function(e) {
    ...
    if (previewBtn) {
        ...
        if (window.ResourcePreview && typeof window.ResourcePreview.previewResource === 'function') {
            window.ResourcePreview.previewResource(resourceId, counselorResourcesData);
        } else {
